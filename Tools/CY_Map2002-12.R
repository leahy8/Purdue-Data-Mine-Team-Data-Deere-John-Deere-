...
CY_2002 <-subset(CY_2002, 50 < CY_2002$Value)
CY_2002 <-subset(CY_2002, CY_2002$Value < 200)
...
CY_2002_Map <- CY_2002_Mapbase %+% CY_2002 +
  ...
  stat_summary_2d(data = NULL)+
  scale_fill_gradientn(name = "Yield (Bushels/Acre)", breaks=c(50,100,150,200), labels=c('50', '100','150', '200'), colours= colormap) +
...
CY_2003 <-subset(CY_2003, 50 < CY_2003$Value)
CY_2003 <-subset(CY_2003, CY_2003$Value < 200)
...
CY_2004 <-subset(CY_2004, 50 < CY_2004$Value)
CY_2004 <-subset(CY_2004, CY_2003$Value < 200)
...
CY_2004_Map <- CY_2004_Mapbase %+% CY_2004 +
  aes(x = Longitude, y = Latitude, z = Value, size = 10, alpha = 0.03) +
  stat_summary_2d()+
  scale_fill_gradientn(ame = "Yield (Bushels/Acre)",breaks=c(50,100,150,200),labels=c('50','100','150','200'), colours= colormap) +
  labs(x = "Longitude", y = "Latitude") +
...
CY_2005 <-subset(CY_2005, 50 < CY_2005$Value)
CY_2005 <-subset(CY_2005, CY_2005$Value < 200)
...
CY_2005_Map <- CY_2005_Mapbase %+% CY_2005 +
  aes(x = Longitude, y = Latitude, z = Value, size = 10, alpha = 0.03) +
  stat_summary_2d()+
  scale_fill_gradientn(name = "Yield (Bushels/Acre)", colours= colormap, breaks=c(50,100,150,200),labels=c('50','100','150','200')) +
  labs(x = "Longitude", y = "Latitude") +
...
CY_2006 <-subset(CY_2006, 50 < CY_2006$Value)
CY_2006 <-subset(CY_2006, CY_2006$Value < 200)
...
CY_2007 <-subset(CY_2007, 50 < CY_2007$Value)
CY_2007 <-subset(CY_2007, CY_2007$Value < 200)
...
CY_2008 <-subset(CY_2008, 50 < CY_2008$Value)
CY_2008 <-subset(CY_2008, CY_2008$Value < 200)
CY_2008_Mapbase <- ggmap(get_googlemap(center = NorthMW_center, zoom = 6, alpha = 0.5))

colormap <- c("Blue", "Green", "Yellow", "Red")
CY_2008_Map <- CY_2008_Mapbase %+% CY_2008 +
  aes(x = Longitude, y = Latitude, z = Value, size = 10, alpha = 0.03) +
  stat_summary_2d(data = NULL, drop = TRUE, )+
  scale_fill_gradientn(name = "Yield (Bushels/Acre)", colours= colormap, breaks=c(50,100,150,200),labels=c('50','100','150','200')) +
  labs(x = "Longitude", y = "Latitude") +
...
CY_2009 <-subset(CY_2009, 50 < CY_2009$Value)
CY_2009 <-subset(CY_2009, CY_2009$Value < 200)
...
CY_2010 <-subset(CY_2010, 50 < CY_2010$Value)
CY_2010 <-subset(CY_2010, CY_2010$Value < 200)
...
CY_2011 <-subset(CY_2011, 50 < CY_2011$Value)
CY_2011 <-subset(CY_2011, CY_2011$Value < 200)
...
CY_2012 <-subset(CY_2012, 50 < CY_2012$Value)
CY_2012 <-subset(CY_2012, CY_2012$Value < 200)
