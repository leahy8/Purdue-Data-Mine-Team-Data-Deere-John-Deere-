...
baseparam <- ldply(paramJSON, data.frame, stringsAsFactors=F)
param <- subset(baseparam, state=='Wisconsin' & value != 0)
param2 <- subset(baseparam, state=='Iowa' & value != 0) 
param3 <- subset(baseparam, state=='Michigan' & value != 0)
param4 <- subset(baseparam, state=='Minnesota' & value != 0)
...
# Remove 0 yield
myDFF <- subset(myDFF, Yield != 0)
myDF2F <- subset(myDF2F, Yield != 0)
myDF3F <- subset(myDF3F, Yield != 0)
myDF4F <- subset(myDF4F, Yield != 0)
